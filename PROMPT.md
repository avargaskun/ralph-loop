...
- **Never preemptively skip or disable a test based on code inspection.** If you read the source code and believe a test will fail, run the test anyway and let the actual result prove or disprove your analysis. Code inspection can be wrong — the runtime behavior is the ground truth. Only add skip conditions after observing an actual test failure, and document the failure output in Observations as evidence.
- **Never mark a task `[x]` unless you actually did it.** A task that was skipped, blocked, or not run is NOT done — leave it as `- [ ]`. The coordinating agent will spawn another iteration to finish it. Marking undone tasks as complete to "move on" corrupts the plan and wastes iterations when the gap is discovered later. If you cannot complete a task (blocked by infrastructure, out of context, etc.), leave it unchecked, explain why in Observations, and output `RALPH_PHASE_COMPLETE` — the next iteration will pick it up.
